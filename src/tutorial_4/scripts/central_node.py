...
class Central:
    ...
    def key_cb(self,data):
        ...
        if data.data == "b":
            ...
            self.robot_state = 7
            self.change_red_blob_pos = True
            rospy.loginfo(rospy.get_caller_id() + " track red ball with fnn")

        elif data.data == "f":
            self.robot_state = 8
            self.change_red_blob_pos = True
            rospy.loginfo(rospy.get_caller_id() + " track red ball with CMAC")

        elif data.data[0] == "f":
            self.robot_state = 8
            self.change_red_blob_pos = False
            coord = data.data[1:].split()
            self.red_blob_x = int(coord[0])
            self.red_blob_y = int(coord[1])
            rospy.loginfo(rospy.get_caller_id() + " track red ball with CMAC, stop using real red blob, test coordiates [%d, %d]" % (self.red_blob_x, self.red_blob_y))
